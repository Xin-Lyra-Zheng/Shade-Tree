"""
Standalone copy of the standard ShadeTree-boosting dataset preparation logic.

Purpose:
- Enable real-dataset testing without modifying the original ShadeTree directory.
- Prefer the workspace-root dataset/ directory by default.
"""
...
from _binning import BinningConfig, PercentileBinner
...
def _load_sklearn_dataset(data_dir, config):
    ...
    if target_col not in df.columns:
        if "target" in df.columns:
            ...
            raise KeyError(f"Cannot find target column {target_col!r}.")
    df.dropna(inplace=True)
    return df, None
...
def prepare_data(args):
    ...
    cfg = BinningConfig(
        strategy=str(getattr(args, "binning_strategy", "quantile")),
